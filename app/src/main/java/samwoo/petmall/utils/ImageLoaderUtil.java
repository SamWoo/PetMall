...
public class ImageLoaderUtil implements ImageLoader {
    ...
    private static void loadNormal(String url, ImageView img) {
        Glide.with(mContext).load(url).dontAnimate().diskCacheStrategy(DiskCacheStrategy.SOURCE).into(img);
    }
    ...
    private static void loadCache(String url, ImageView img) {
        ...
        }).load(url).placeholder(R.drawable.default_pet_info_icon).dontAnimate().diskCacheStrategy(DiskCacheStrategy.SOURCE).into(img);
